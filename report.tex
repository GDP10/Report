...
Pilots need to be notified of hazards and events that may require them to make changes to their flight plan. Examples of such events include: closed runways, restricted airspaces, extreme weather conditions, and even the presence of migrating birds through an airspace. Currently, flight planners receive such information by pulling all current NOTAMs, or `Notice to Airmen', and must manually sort through them to find only the notices which are relevant to their flight, which is extremely time consuming. The efficiency of this task could be improved dramatically if the filtering were performed automatically, using defined geographic regions and time frames. The aeronautics industry favours the WS-Notification family of standards for publish/subscribe systems, and so an early constraint placed on the system by the customer required us to work to either this standard or to a similar competing standard named WS-Eventing.
...
\end{center}
\caption[UML diagram highlighting some features of the AIXM conceptual model]{UML diagram highlighting some features of the AIXM conceptual model \cite{aixm}. The model gives information to pilots such as the number of runways at an airport and their sizes. If there are no runways large enough for the plane then this information is important so that the flight plan can be changed.}
\label{fig:aixm}
\end{figure}
...
\end{center}
\caption[Publish/Subscribe Architecture with Broker]{Publish/Subscribe Architecture with Broker. Multiple producers can all publish to the same broker and the broker then becomes a front-end for these producers to transfer messages to subscribers. Consumers can subscribe using string-based topics or by using filters, depending on the implementation.}
\label{fig:pubsub}
\end{figure}
...
\end{tabular}
\caption{Skills Audit Table}
\label{tab:skillsAudit}
\end{table}
...
\label{sec:plan_version_control}
...
\begin{figure}[h]
\centering
  \includegraphics[width=0.7\linewidth]{pull_request.png}
  \caption[Example pull request]{Example pull request. When a team member wants to merge in their branch, they open a pull request which other team members can write issues on. These ensure that the team is satisfied with eachother's work throughout the project. The pull requests also trigger automated tests on Travis to check for any new bugs in the new code.}
...
\begin{table}[h]
    ...
    \end{tabular}
\caption[Sprint 2 user story table]{Sprint 2 user story table. This task breakdown laid out tasks to be completed row by row but the order of columns could be done in any order. In practice the team found it very simple to move from the first row straight to the full configurable code.}
\label{tab:sprint2}
\end{table}
...
\end{tikzpicture}
\caption[System Overview]{System overview. The system has been designed to be scalable, multiple JMS publishers can publish to ActiveMQ and then multiple WSN brokers can become subscribers to ActiveMQ. Each individual broker can then have a number of consumers subscribed to them using spatial and temporal filtering.}
\label{fig:system_overview}
\end{figure}

\begin{figure}[h]
  \includegraphics[width=\linewidth]{aes_class_diagram.png}
  \caption[System class diagram]{System class diagram. The system uses interfaces which are automatically generated from WSN WSDLs from Oasis.}
  \label{fig:aes_class_diagram}
\end{figure}

The completed system consists of two parts: a producer, which parses NOTAM data and publishes it to a JMS broker (ActiveMQ, in this project), and a basic WS-BrokeredNotification implementation that allows consumers to retrieve messages from this JMS broker via the WS-Notification protocol suite. A block diagram giving an overview of this system is provided in figure~\ref{fig:system_overview}, and a class diagram is given in figure~\ref{fig:aes_class_diagram}.
...
\begin{figure}
  \includegraphics[width=\linewidth]{consumer_class_diagram.png}
  \caption[Consumer class diagram]{Consumer class diagram. The consumer also has a set of interfaces generated from an Oasis WSDL. The interface contains a single method to notify a consumer when they have a message.}
  \label{fig:consumer_class_diagram}
\end{figure}

Although the consumer was out of this project's scope, a basic implementation was still produced, in order to demonstrate the system's functionality. A class diagram for this separate project is given in figure~\ref{fig:consumer_class_diagram}. Note that this project does not directly reference any of the classes from the main project; instead, it uses the published WSDL for the project's service, along with JAX-WS, to connect to the service.

\section{Summary of Explored Technologies}
\label{sec:explored}

Given the exploratory goals of the project, information gained about technologies not used is potentially just as valuable to the customer as the final system itself. Overall, it was found that no complete WSN implementations existed, within Java message brokers that were capable of satisfying all of the customer's requirements.

Apache ActiveMQ was one of the main message brokers that the customer suggested being investigated, as it officially listed support for the WS-Notification protocol. Despite this, it had no such implementation and the documentation for which, whilst listed, was non-existent.

Apache CXF was found to have a limited implementation but it was unlisted and undocumented, making relying upon it a risky proposition, since the lack of officially declared support could mean it was untested or that it could be removed at any time. Additionally, message filtering was limited to filtering based on topics, meaning it would be unable to satisfy all of the customer's requirements.

JBoss A-MQ, a fork of ActiveMQ, had the same implementation as Apache CXF. It was initially promising, with the customer expressing high expectations for it, but it was found that it could only perform filtering based on string matching, so was also unable to meet the requirements.

One potential solution for this was to fork JBoss A-MQ and to manual implement a more capable filtering system, which may be an option the customer finds worth exploring in the future. However, in the limited time available for this project, it was instead opted to use JMS for filtering, in combination with ActiveMQ and a basic WS-BrokeredNotification implementation, as described previously.
...
Despite these successes, there were some areas that could have been improved. As well as the scheduled meetings, there were regular workshop meetings in labs where team members would work on code together and discuss issues with each other. There was no formal format to these workshops which meant a lot of the dialogue was conversational and no record was kept. For absent members of the workshop meetings it was therefore difficult to catch up on what had happened in these workshop meetings without talking to those who attended. This meant that in some cases there were miscommunications about what work needed to be done and some time was wasted duplicating work or working on technologies that had been abandoned.

Those who attended the workshop meetings found them useful and productive. Most of the team preferred working together on code rather than having meetings to discuss the work then splitting off to work individually. Although tasks would be assigned to individual team members when able, they were nonetheless very linked. The format of the workshop meant that as a team member was writing their code, they could easily discuss with other team members about other parts of the code and make sure that the modules would work together. This was particularly useful for the team member who wrote tests for the code as they had to understand how the various classes had to work together.
...
\subsection{Airport Data Retrieval}
\label{sec:future_airport}

Currently in the AES system, NOTAM enrichment is performed from a processed XML file originally obtained from Snowflake Software's Go Publisher WFS, containing information on all airports listed in the WFS. This could be changed to query the web feature service directly for each NOTAM, to ensure the data is up to date. This would introduce a significant bottleneck, so it would have to be combined with some form of caching (for example, storing all airport data locally and updating it by querying the WFS once each day). If possible, a better solution would be to combine caching with some form of web cache validation (provided the WFS supports it), where client requests are conditional on the content having changed, if not, the WFS doesn't need to send the usual full response.
...
\begin{tabularx}{\textwidth}{|X|X|} \hline
  ...
  \fullref{sec:introduction}                          & Charles Sherman\par Peter Prince   \\
  ...
  \fullref{sec:intro_goals}                           & Peter Prince                       \\
  \fullref{sec:intro_deliverables}                    & Peter Prince                       \\ \hline
  ...
  \fullref{sec:bg_wfs}                                & Andrew Sullivan                    \\
  ...
  \fullref{sec:bg_gml}                                & Andrew Sullivan                    \\ \hline
  \fullref{sec:planning}                              & Peter Prince                       \\
  \fullref{sec:plan_software_development_methodology} & Peter Prince                       \\
  ...
  \fullref{sec:plan_version_control}                  & James Robinson                     \\
...
\begin{tabularx}{\textwidth}{|X|X|} \hline
  ...
  \fullref{sec:explored}                          & Andrew Sullivan \\ \hline
  \fullref{sec:critical_evaluation}                   & Peter Prince                       \\
  \fullref{sec:eval_project_management}               & Andrew Sullivan\par Charles Sherman\\
  ...
  \fullref{sec:future_compilation}                    & Peter Prince                       \\
  \fullref{sec:future_airport}                    & Andrew Sullivan                     \\ \hline
\end{tabularx}
