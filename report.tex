...
In order to perform spatial filtering, latitude and longitude data for every airspace and airfield would be required, as NOTAMs lack information of this level of detail. The information required for temporal filtering is featured in each NOTAM, and only conversion to a usable timestamp format was required.
...
The sprint meeting for the fourth sprint consisted of bringing up to speed members of Snowflake in attendance who had not been notified about JBoss A-MQ. As well as the standard customer representatives of Eddie and Adrian, Alexis Brooker also attended the sprint meeting, and after discussing the need for GPS locations in order to perform spatial filtering Alexis provided us with access to a HTTP-GET service which returns latitude and longitude locations for all European airfields. In an ideal world, calling this service would be implemented into AES itself, however given the time remaining on the project and the status of the product as a proof of concept, we elected to simply query a large set of co-ordinates and hard-coding them into the system for use in spatial queries, knowing that the system would possess the capacity to directly call the service and utilise the data if more time was spent on it.

After JBoss A-MQ was proved to be a dead end, a new structure for the system was decided upon, with only the consumer side using WS-Notifications to communicate. All remaining communication between brokers and producers was changed to rely on JMS, with converters between WS-Notification topics and JMS filters becoming a primary focus for the sprint. 
