...
For the first sprint the goal was to have a basic push notification system running that could publish and recieve an XML. As well as this we had to set up our developement environment and tools for the project.
...
To fufill this goal we had to set up Apache ActiveMQ\ref{activeMQ} and find a way to give the broker a message and a way of displaying the message so we know it had been recieved. 

\subsubsection{Work Allocation}

As the sprint goal was very simple it was difficult to seperate the work between team members. James Robinson did the majority of the coding work while everyone else in the group assisted by researching the possible tools we could use. This was in effect an extension of pair-programming, where two people working on a single machine take turns in programming while the other checks for mistakes.
