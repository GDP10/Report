...
The customer had no preference for what language the project was written in. The team chose Java because of the familiarity the team already had with it. The team worked in the IDE Eclipse, and used the build system Maven to handle library dependencies, as it has automatic dependency resolution and plugins for code generation. For version control the team used GitHub.
...
There were no automated tests written for the first sprint. Since values were hard-coded for the broker address and the message topics, there was only one route through the code. For this sprint the team tested the code manually and did not write automated unit or system-level tests. As the code expanded in future sprints, automated tests were added to check for errors in the code when changes were made and to demonstrate to the customer that the project was working.
...
In the planning meeting for this sprint, the team was made aware of JBoss A-MQ by Snowflake. JBoss A-MQ is a fork of Apache ActiveMQ that also contains the Apache CXF implementation of WS-Notifications. It was hoped that swapping to using this system instead of the code already written would allow the team to implement more complex filtering on messages. 
...
Unfortunately the team was unable to implement WS-Notifications in this sprint, however the code for filtering and enriching the data was completed. The task of creating a WS-Notification front end was much larger than expected and only assigning a single team member to work on it meant that the team couldn't complete the task in time.
...
%Please mention the fact we didn't apply for funding to do this at the beginning as we didn't consider this kind of testing at the start. By the time we realised it would be useful we were past the funding deadline. This is just to clarify what I wrote in the conclusion bit above.
Assigned to James
